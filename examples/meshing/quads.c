...
add_radial_scalar(FmsDataCollection dc, FmsComponent surface,
                  const char *name, const double *gc, int order, int nverts, int nedges,
                  int nquads) {
  int i, ndofs;
  float *data;
...
main(int argc, char *argv[]) {
  ...
  FmsDomainAddEntities(domains[0], FMS_QUADRILATERAL, NULL, FMS_INT32, quad_edge,
                       nquads);
  ...
  cptr = append_coords_face(order, cptr, global_coords, edge_vert, quad_edge,
                            nquads, 0.01);
