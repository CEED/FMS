...
compute_faces(const int *dims, int *nfaces) {
  ...
  *nfaces = F1 + F2 + F3;
  ...
  for(i = 0; i < *nfaces; i++)
    printf("face %d: %d %d %d %d\n", i, faces[4*i], faces[4*i+1], faces[4*i+2],
           faces[4*i+3]);
#endif
...
*/
double *
append_coord_dofs_verts(double *dest, const double *verts, int nverts,
                        int component) {
  int i;
  for(i = 0; i < nverts; ++i)
...
WriteTimeStep(const char *filename, const char *protocol, const int dims[3],
  ...
  FmsDomainAddEntities(domains[0], FMS_QUADRILATERAL, NULL, FMS_INT32, faces,
                       nfaces);
#ifdef ADD_CELLS
  /* Set the cells */
  FmsDomainSetNumEntities(domains[0], FMS_HEXAHEDRON, FMS_INT32, ncells);
  FmsDomainAddEntities(domains[0], FMS_HEXAHEDRON, NULL, FMS_INT32, cells,
                       ncells);
#endif
  ...
  FmsFieldDescriptorSetComponent(coords_fd,
                                 volume); /* define coords over surface */
  ...
  add_radial_scalar(dc, volume, "r1", 1, dims, verts, nverts, edges, nedges,
                    faces, nfaces, ncells);
  add_radial_scalar(dc, volume, "r2", 2, dims, verts, nverts, edges, nedges,
                    faces, nfaces, ncells);
  add_radial_scalar(dc, volume, "r3", 3, dims, verts, nverts, edges, nedges,
                    faces, nfaces, ncells);
  ...
  /* Write coordinate dofs as Point3D so we can plot them. */
  {
    ...
    if(f != NULL) {
      fprintf(f, "X Y Z dof\n");
      for(i = 0; i < ndofs; ++i)
        fprintf(f, "%lg %lg %lg %d\n", coord_data[i],coord_data[ndofs+i],
                coord_data[2*ndofs+i], i);
      fclose(f);
    }
